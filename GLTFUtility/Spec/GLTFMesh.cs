...
namespace Siccity.GLTFUtility {
	...
	/* [Preserve removed for IL2CPP build] */ public class GLTFMesh {
				...
				public MeshData(GLTFMesh gltfMesh, GLTFAccessor.ImportResult[] accessors, GLTFBufferView.ImportResult[] bufferViews) {
					...
					if (gltfMesh.primitives.Count == 0) {
						...
						for (int i = 0; i < gltfMesh.primitives.Count; i++) {
							...
							// Draco-compressed primitives need the GLTFUtilityDracoLoader
							// upstream extension; we don't ship it. The plugin's own GLBs
							// never use KHR_draco_mesh_compression, so we just refuse here.
							if (primitive.extensions != null && primitive.extensions.KHR_draco_mesh_compression != null) {
								throw new NotSupportedException("KHR_draco_mesh_compression is not supported in this build.");
							}
							// Load normal mesh
							else {
								...
								if (primitive.indices.HasValue) {
									// Explicit Enumerable.Reverse to avoid an IL2CPP overload that
									// resolves Array.Reverse (void return) instead of LINQ's Reverse.
									int[] rawIndices = accessors[primitive.indices.Value].ReadInt();
									submeshTris.Add(new List<int>(System.Linq.Enumerable.Reverse(rawIndices).Select(x => x + vertStartIndex)));
